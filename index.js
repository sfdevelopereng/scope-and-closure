...
function inning(){
  let points = Math.ceil(Math.random() * 2)
...
function finalScore(track, innings){
  const score = {Home: 0,
  Away: 0}
  for(let i = 0; i < innning; i++){
    points = track();
    score.Home += points
  }
  return score
...
function getInningScore(getPoints) {
  score.Home = getPoints();
  score.Away = getPoints();
